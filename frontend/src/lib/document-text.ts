import { findCase, REQUIRED_FIELDS, type VerificationCase } from "./case-data";
...
/** Preview for an attachment linked to a case; null when no case exists yet. */
export function documentBodyForCase(caseRef: string | undefined, role: "SI" | "BL" | "other") {
  if (!caseRef || role === "other") return null;
  const verificationCase = findCase(caseRef);
  if (!verificationCase) return null;
  return caseDocumentBody(verificationCase, role);
}
