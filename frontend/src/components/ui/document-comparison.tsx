...
function DocumentPane({
  ...
  const originalLikelyPdf = /pdf/i.test(evidence.contentType || evidence.name);
  ...
  useEffect(() => {
    if (position.mode === "original" && !sourceUrl && !sourceError && evidence.documentId) onLoadSourceRef.current();
  ...
  const download = async () => {
    ...
    if (position.mode === "original") {
      try {
        if (!evidence.documentId || !evidence.caseId) throw new Error("The original file is not available for download.");
        ...
        setDownloadError(error instanceof Error ? error.message : "The original file could not be downloaded.");
        return;
      }
    }
    ...
    const link = document.createElement("a");
    link.href = url;
    link.download = `${evidence.name.replace(/\.[a-z0-9]+$/i, "")}-extracted.txt`;
    document.body.appendChild(link);
    link.click();
    link.remove();
    URL.revokeObjectURL(url);
  ...
  return (
    <article className={cn("flex min-h-[360px] min-w-0 flex-col overflow-hidden rounded-xl border bg-surface shadow-glass", role === "BL" && evidence.extractedValue === null ? "border-review-200/80" : "border-line")}>
      <header className="flex flex-wrap items-start justify-between gap-3 border-b border-line px-3 py-3 sm:px-4">
        <div className="min-w-0 flex-1">
          <p className="flex flex-wrap items-center gap-2">
            <span className={cn("rounded px-1.5 py-0.5 font-mono text-[10.5px] font-semibold text-white", displayRole === "BL" && evidence.extractedValue === null ? "bg-gradient-to-b from-review-500 to-review-700" : "bg-gradient-to-b from-brand-500 to-brand-700")}>{displayRole === "other" ? "FILE" : displayRole}</span>
            <span className="truncate text-[13px] font-semibold text-ink-900">{displayRole === "SI" ? "Shipping Instruction" : displayRole === "BL" ? "Bill of Lading" : evidence.name}</span>
            {evidence.scanned && <span className="inline-flex items-center gap-1 rounded-md bg-review-50 px-1.5 py-0.5 text-[10.5px] font-medium text-review-700 ring-1 ring-inset ring-review-200"><ScanLine className="size-3" strokeWidth={2.25} /> Scanned</span>}
          </p>
          <p className="mt-1 flex items-center gap-1.5 truncate text-[11.5px] text-ink-400"><FileText className="size-3 shrink-0 text-ink-300" strokeWidth={2} />{evidence.name} · {evidence.pages} page{evidence.pages === 1 ? "" : "s"}</p>
        </div>
          <button type="button" onClick={() => void download()} className="btn-glass shrink-0 px-2 py-1 text-[11px]" aria-label={`Download ${evidence.name}`}><Download className="size-3" strokeWidth={2} /> Download</button>
      </header>
      ...
      <div className="flex flex-wrap items-center justify-between gap-2 border-b border-line bg-surface/60 px-3 py-2 sm:px-4">
        <div role="tablist" aria-label={`${role} document view`} className="flex rounded-lg border border-line bg-canvas/40 p-0.5">
          <button type="button" role="tab" aria-selected={position.mode === "original"} onClick={() => { onChange({ mode: "original" }); if (!sourceUrl) onLoadSource(); }} disabled={!originalLikelyPdf && !evidence.documentId} className={cn("rounded-md px-2.5 py-1 text-[11px] font-medium", position.mode === "original" ? "bg-surface text-brand-700 shadow-glass" : "text-ink-400 hover:text-ink-700", (!originalLikelyPdf && !evidence.documentId) && "cursor-not-allowed opacity-40")}>Original PDF</button>
          <button type="button" role="tab" aria-selected={position.mode === "text"} onClick={() => onChange({ mode: "text" })} className={cn("rounded-md px-2.5 py-1 text-[11px] font-medium", position.mode === "text" ? "bg-surface text-brand-700 shadow-glass" : "text-ink-400 hover:text-ink-700")}>Extracted Text</button>
        </div>
        {position.mode === "original" && (
          <div className="flex items-center gap-1 text-[11px] text-ink-500">
            <button type="button" className="grid size-6 place-items-center rounded-md hover:bg-surface" onClick={() => onChange({ zoom: Math.max(50, position.zoom - 10) })} aria-label="Zoom out"><Minus className="size-3" /></button>
            <button type="button" className="tabular min-w-[42px] rounded px-1 text-center font-medium hover:bg-surface" onClick={() => onChange({ zoom: 100 })} title="Click to reset zoom to 100%">{position.zoom}%</button>
            <button type="button" className="grid size-6 place-items-center rounded-md hover:bg-surface" onClick={() => onChange({ zoom: Math.min(200, position.zoom + 10) })} aria-label="Zoom in"><Plus className="size-3" /></button>
          </div>
        )}
      </div>
      ...
      <div className="min-h-0 flex-1 bg-canvas/60">
        {position.mode === "original" ? (
          <div className="relative flex h-[min(65vh,720px)] min-h-[380px] flex-col">
            {sourceUrl ? (
              <iframe title={`${evidence.name} original PDF`} src={`${sourceUrl}#page=${Math.max(1, position.page)}&zoom=${position.zoom}`} className="min-h-0 flex-1 border-0 bg-white" />
            ) : (
              <div className="flex flex-1 flex-col items-center justify-center gap-2.5 p-6 text-center text-[12.5px] text-ink-500">
                <FileText className="size-8 text-ink-300" strokeWidth={1.5} />
                <p className="font-semibold text-ink-800">{sourceError || (unreadable ? "Original document is unreadable." : "Load the original file to inspect the source.")}</p>
                <p className="max-w-sm text-[11.5px] text-ink-400">If the original file is unavailable or cannot be loaded, inspect the parsed text lines instead.</p>
                <div className="mt-2 flex flex-wrap items-center justify-center gap-2">
                  {evidence.documentId && !sourceError && (
                    <button type="button" onClick={onLoadSource} className="btn-primary px-3 py-1.5 text-[12px]">Load original PDF</button>
                  )}
                  <button type="button" onClick={() => onChange({ mode: "text" })} className="btn-glass px-3 py-1.5 text-[12px]">View extracted text</button>
                </div>
              </div>
            )}
            <div className="flex items-center justify-center gap-2 border-t border-line bg-surface/70 px-3 py-2">
              <button type="button" onClick={() => onChange({ page: Math.max(1, position.page - 1) })} disabled={position.page <= 1} className="grid size-7 place-items-center rounded-md text-ink-500 hover:bg-surface disabled:opacity-30" aria-label="Previous page"><ChevronLeft className="size-4" /></button>
              <label className="flex items-center gap-1 text-[11px] text-ink-500"><span className="sr-only">Page</span><input aria-label="Current page" type="number" min={1} max={pageCount} value={position.page} onChange={(event) => onChange({ page: Math.min(Math.max(1, Number(event.target.value) || 1), pageCount) })} className="field-glass w-12 px-1.5 py-1 text-center text-[11px]" /> / {pageCount}</label>
              <button type="button" onClick={() => onChange({ page: Math.min(pageCount, position.page + 1) })} disabled={position.page >= pageCount} className="grid size-7 place-items-center rounded-md text-ink-500 hover:bg-surface disabled:opacity-30" aria-label="Next page"><ChevronRight className="size-4" /></button>
            </div>
          </div>
        ) : (
          <div ref={textRef} onScroll={(event) => onChange({ textScrollTop: event.currentTarget.scrollTop })} className="h-[min(65vh,720px)] min-h-[380px] overflow-y-auto p-3 sm:p-4">
            <pre className="mx-auto max-w-[760px] whitespace-pre-wrap break-words rounded-xl border border-line bg-surface px-3 py-4 font-mono text-[11.5px] leading-[1.85] text-ink-700 shadow-glass sm:px-5">
              {unreadable && <span className="mb-3 block rounded-lg border border-review-200 bg-review-50/70 px-3 py-2 font-sans text-[12px] font-medium text-review-700">This document could not be read reliably. Treat extracted values as unresolved until a reviewer supplies a decision.</span>}
              {lines.length ? lines.map((line, index) => <span key={index} className={cn("block rounded px-1.5", problems.has(index) && (role === "BL" ? "bg-review-50 font-semibold text-review-700 ring-1 ring-inset ring-review-200" : "bg-brand-50 font-semibold text-brand-700 ring-1 ring-inset ring-brand-200"))}>{line || " "}</span>) : <span className="block text-ink-400">No extracted text is available.</span>}
            </pre>
          </div>
        )}
      </div>

      <div className="border-t border-line px-3 py-2.5 sm:px-4"><p className="text-[12px]"><span className="text-ink-400">{problemField} read as: </span><span className={cn("font-semibold", evidence.extractedValue ? "text-ink-900" : "text-review-700")}>{evidence.extractedValue ?? "[unreadable]"}</span></p>{downloadError && <p className="mt-1 text-[11px] text-review-700">{downloadError}</p>}</div>
  </article>
  );
}
