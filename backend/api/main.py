...
def create_app(runtime: Runtime | None = None) -> FastAPI:
    ...
    app.add_middleware(
        CORSMiddleware,
        allow_origins=[settings.dashboard_base_url.rstrip("/"), "http://localhost:5173"],
    ...
    def require_reviewer(
        ...
        if not claims or not runtime.repository.is_reviewer(claims["uid"]):
    ...
    def list_cases(
        ...
        return runtime.repository.list_cases(limit=limit, status=status)

    @app.get("/api/cases/{case_id}", dependencies=[Depends(require_reviewer)])
    def get_case(case_id: str) -> dict[str, Any]:
        case = get_case_or_404(case_id)
        case["documents"] = runtime.repository.list_documents(case_id)
        return case
    ...
    @app.get("/api/knowledge-base/preview/{filename}")
    def preview_knowledge_base(filename: str) -> HTMLResponse:
        clean_name = re.sub(r"[^a-zA-Z0-9_\-]", "", filename)
        file_path = runtime.knowledge_publisher.local_dir / f"{clean_name}.html"
        if not file_path.exists():
            md_path = runtime.knowledge_publisher.local_dir / f"{clean_name}.md"
            if not md_path.exists():
                raise HTTPException(404, "Preview document not found")
            content = md_path.read_text(encoding="utf-8")
            html_body = f"<pre style='padding:20px;font-family:monospace;'>{content}</pre>"
    ...
    def gmail_oauth_callback(code: str, state: str) -> dict[str, str]:
        try:
            payload = verify_state(state, settings.telegram_webhook_secret)
...
def _handle_telegram_update(runtime: Runtime, update: dict[str, Any]) -> None:
    ...
    if text.startswith("/start"):
        runtime.telegram.send_message(
            chat_id,
            "<b>ClassAll Document Review</b>\n"
            "Use /newcase, add #TOKEN to each SI/BL upload, then /submit TOKEN.",
        )
        return
    ...
    if text.startswith("/submit"):
        ...
        if not case or str(case.get("owner_chat_id")) != chat_id:
            ...
            return
        runtime.repository.update_case(case_id, {"processing_state": "QUEUED"})
        runtime.publisher.publish({"case_id": case_id})
    ...
    if document:
        ...
        runtime.repository.update_case(case["case_id"], {"processing_state": "DRAFT"})
        runtime.telegram.send_message(
            chat_id,
